 ...
 * @return array
 */
function kasiaikod_body_classes( $classes ) {
...
add_filter( 'body_class', 'kasiaikod_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function kasiaikod_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'kasiaikod_pingback_header' );
 ...
 * @return array
 */
function kasiaikod_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'kasiaikod_filter_script_loader_tag', 10, 2 );
 ...
 */
function kasiaikod_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function kasiaikod_add_body_style() {
	...
	if ( ! kasiaikod_is_amp() ) {
		...
		// Preload content.css.
		$preloads['kasiaikod-content'] = kasiaikod_get_preload_stylesheet_uri( $wp_styles, 'kasiaikod-content' );

		// Preload sidebar.css and widget.css.
		if ( is_active_sidebar( 'sidebar-1' ) ) {
			$preloads['kasiaikod-sidebar'] = kasiaikod_get_preload_stylesheet_uri( $wp_styles, 'kasiaikod-sidebar' );
			$preloads['kasiaikod-widgets'] = kasiaikod_get_preload_stylesheet_uri( $wp_styles, 'kasiaikod-widgets' );
		}

		// Preload comments.css.
		if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
			$preloads['kasiaikod-comments'] = kasiaikod_get_preload_stylesheet_uri( $wp_styles, 'kasiaikod-comments' );
		}

		// Preload front-page.css.
		global $template;
		if ( 'front-page.php' === basename( $template ) ) {
			$preloads['kasiaikod-front-page'] = kasiaikod_get_preload_stylesheet_uri( $wp_styles, 'kasiaikod-front-page' );
		}
...
add_action( 'wp_head', 'kasiaikod_add_body_style' );
 ...
 */
function kasiaikod_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'kasiaikod_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function kasiaikod_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'kasiaikod_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'kasiaikod_add_nav_menu_aria_current', 10, 2 );
